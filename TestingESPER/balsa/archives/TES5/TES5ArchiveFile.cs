...
public class TES5ArchiveFile : ArchiveFile
{
    ...
    public override bool hasDirectoryNames => header.archiveFlags.HasFlag("Include Directory Names");

    public override bool hasFileNames => header.archiveFlags.HasFlag("Include File Names");

    public override bool embedFileNames => header.archiveFlags.HasFlag("Embed File Names");
    ...
    internal override int GetFileRecordIndex(FileRecord fileRecord)
    {
        ...
        var index = 0;
        for (var i = 0; i < folderRecords.Length; i++)
        {
    ...
    {
        var folderRecord = Array.Find(folderRecords, f => { return f.name == folderPath; });
