...
using System.IO;
using TestingESPER.balsa.setup;
using System.Linq;
using TestingESPER.balsa.stringtables;
using System;

namespace TestingESPER.esper.setup {
    public class ResourceManager {
        private readonly Session session;
        private readonly List<string> allArchives;
        private readonly balsa.setup.Game balsaGame;
        ...
        private balsa.setup.Game ResolveBalsaGame() {
            var targetGameKey = session.game.abbreviation;
            var fields = typeof(balsa.setup.Games).GetFields();
            foreach (var field in fields)
                if (field.Name == targetGameKey)
                    return (balsa.setup.Game)field.GetValue(null);
            return null;
        }
