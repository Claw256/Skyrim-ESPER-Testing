namespace TestingESPER.IniParser.Parser;
...
public sealed class StringBuffer
{
    ...
    public int Count => _bufferIndexes.size;

    public bool IsEmpty => _bufferIndexes.IsEmpty;

    public bool IsWhitespace
    {
        get
        {
            var startIdx = _bufferIndexes.start;
    ...
    public char this[int idx] => _buffer[idx + _bufferIndexes.start];
    ...
    public Range FindSubstring(string subString, int startingIndex = 0)
    {
        ...
        if (subStringLength <= 0 || Count < subStringLength) return new Range();

        startingIndex += _bufferIndexes.start;

        // Search the first char of the substring
        for (var firstCharIdx = startingIndex; firstCharIdx <= _bufferIndexes.end; ++firstCharIdx)
        {
            if (_buffer[firstCharIdx] != subString[0]) continue;

            // Fail now if the substring can't fit given the size of the
            // buffer and the search start index
            if (firstCharIdx + subStringLength - 1 > _bufferIndexes.end) return new Range();

            var isSubstringMismatch = false;
            // Check if the substring matches starting at the index
            for (var currentIdx = 0; currentIdx < subStringLength; ++currentIdx)
            ...
            if (isSubstringMismatch) continue;
    ...
    public bool ReadLine()
    {
        ...
        _buffer.Clear();
        var c = _dataSource.Read();

        // Read until new line ('\n') or EOF (-1)
        while (c != '\n' && c != -1)
        {
            if (c != '\r') _buffer.Add((char) c);
    ...
    public bool StartsWith(string str)
    {
        ...
        var strIdx = 0;
        var bufferIdx = _bufferIndexes.start;

        for (; strIdx < str.Length; ++strIdx, ++bufferIdx)
            if (str[strIdx] != _buffer[bufferIdx])
                return false;
    ...
    public struct Range
    {
        ...
        public int size
        {
            ...
            set => _size = value < 0 ? 0 : value;
        }

        public int end => _size <= 0 ? 0 : _start + (_size - 1);
        ...
        {
            if (start < 0 || end < 0 || end - start < 0) return new Range();
