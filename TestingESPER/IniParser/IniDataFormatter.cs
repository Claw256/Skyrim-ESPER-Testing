...
using TestingESPER.IniParser.Format;
...
public class IniDataFormatter : IIniDataFormatter
{
        ...
        IniFormattingConfiguration format)
    {
        ...
        if (format.NewLineBeforeSection && sb.Length > 0) sb.Append(format.NewLineString);
        ...
        IniFormattingConfiguration format)
    {
        foreach (var property in properties)
        {
            ...
            if (format.NewLineBeforeProperty) sb.Append(format.NewLineString);

            //Write key and value
            sb.Append(
                $"{property.Key}{format.SpacesBetweenKeyAndAssigment}{scheme.PropertyAssigmentString}{format.SpacesBetweenAssigmentAndValue}{property.Value}{format.NewLineString}");

            if (format.NewLineAfterProperty) sb.Append(format.NewLineString);
        }
    }
    ...
    protected virtual void WriteComments(List<string> comments,
        ...
        IniFormattingConfiguration format)
    {
        foreach (var comment in comments) sb.Append($"{scheme.CommentString}{comment}{format.NewLineString}");
    }
