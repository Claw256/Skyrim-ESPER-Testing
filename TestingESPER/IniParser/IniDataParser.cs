...
using TestingESPER.IniParser.Configuration;
using TestingESPER.IniParser.Exceptions;
using TestingESPER.IniParser.Model;
using TestingESPER.IniParser.Parser;

namespace TestingESPER.IniParser;
...
public class IniDataParser
{
    ...
    {
        var iniData = Configuration.CaseInsensitive
    ...
    public void Parse(TextReader textReader, ref IniData iniData)
    {
        ...
        if (Configuration.ParseComments) CurrentCommentListTemp.Clear();
    ...
    protected virtual bool ProcessComment(StringBuffer currentLine)
    {
        ...
        if (!currentLineTrimmed.StartsWith(Scheme.CommentString)) return false;

        if (!Configuration.ParseComments) return true;
        ...
        if (Configuration.TrimComments) comment.Trim();
    ...
    protected virtual bool ProcessSection(StringBuffer currentLine, IniData iniData)
    {
        ...
        if (Configuration.TrimSections) currentLine.Trim();
        ...
        if (!Configuration.AllowDuplicateSections)
            if (iniData.Sections.Contains(sectionName))
            {
                ...
                var errorFormat =
                    "Duplicate section with name '{0}'. Please see configuration option {1}.{2} to ignore this error.";
                var errorMsg = string.Format(errorFormat,
                    sectionName,
                    nameof(Configuration),
                    nameof(Configuration.SkipInvalidLines));
    ...
    protected virtual bool ProcessProperty(StringBuffer currentLine, IniData iniData)
    {
        ...
        if (key.IsEmpty)
        {
            ...
            var errorFormat =
                "Found property without key. Please see configuration option {0}.{1} to ignore this error";
            var errorMsg = string.Format(errorFormat,
                nameof(Configuration),
                nameof(Configuration.SkipInvalidLines));
        ...
        if (string.IsNullOrEmpty(_currentSectionNameTemp))
        {
            ...
            {
                var errorFormat =
                    "Properties must be contained inside a section. Please see configuration option {0}.{1} to ignore this error.";
                var errorMsg = string.Format(errorFormat,
                    nameof(Configuration),
    ...
    /// </summary>
    private void HandleDuplicatedKeyInCollection(string key,
        string value,
    ...
    public List<string> CurrentCommentListTemp
    {
        get
        {
            if (_currentCommentListTemp == null) _currentCommentListTemp = new List<string>();

            return _currentCommentListTemp;
        }

        internal set => _currentCommentListTemp = value;
    }
    ...
    private readonly StringBuffer _mBuffer = new(256);
